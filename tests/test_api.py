...
def test_create_post_schedule(client):
    ...
    assert c.get(f"/api/posts/{post_id}").status_code == 404
...
def test_patch_scheduled_post(client):
    c, _ = client
    post = c.post("/api/posts", json={"text": "old", "platforms": ["mock"],
                                      "scheduled_at": "2030-01-01T09:00:00Z"}).json()
    patched = c.patch(f"/api/posts/{post['id']}", json={"text": "new text",
                                                        "scheduled_at": "2031-02-02T10:00:00Z"})
    assert patched.status_code == 200
    body = patched.json()
    assert body["text"] == "new text"
    assert body["platforms"] == ["mock"]               # untouched fields preserved
    assert body["scheduled_at"].startswith("2031-02-02")

    # published posts can't be edited
    pub = c.post("/api/posts", json={"text": "x", "platforms": ["mock"], "publish_now": True}).json()
    assert c.patch(f"/api/posts/{pub['id']}", json={"text": "y"}).status_code == 400


def test_delete_remote(client):
    c, store = client
    post = c.post("/api/posts", json={"text": "x", "platforms": ["mock"], "publish_now": True}).json()
    resp = c.post(f"/api/posts/{post['id']}/remote").json()
    assert resp["ok"] is True
    assert resp["outcomes"]["mock"] == "deleted"


def test_api_token_auth(tmp_path, monkeypatch):
    monkeypatch.setenv("SOCIALBOT_API_TOKEN", "sekret")
    from socialbot.api.app import create_app as make_app
    app = make_app(store=Store(str(tmp_path / "tok.db")), with_scheduler=False)
    with TestClient(app) as c:
        assert c.get("/api/health").status_code == 401
        assert c.get("/api/health",
                     headers={"Authorization": "Bearer sekret"}).json()["ok"] is True
        assert c.get("/", headers={"Authorization": "Bearer sekret"}).status_code == 200
